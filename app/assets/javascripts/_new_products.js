$(document).on('turbolinks:load', ()=> {
  ...
  const buildFileField = (num)=> {
    const html = `<div data-index="${num}" class="js-file_group">
                    ...
                    id="product_images_attributes_${num}_src"></input><br>
                  </div>`;
    return html;
  }
  // プレビュー用のimgタグを生成する関数
  const buildImg = (index, url)=> {
    const html = `<div id = "Previews2" data-index="${index}"><img data-index="${index}" src="${url}" width="100px" height="100px"><div class="js-remove">削除</div></div>`;
    return html;
  }

  // file_fieldのnameに動的なindexをつける為の配列Z
...
$(document).on('turbolinks:load', ()=> {
  ...
  $(document).on('drop',".DropArea", function (e) {
    e.preventDefault();
    $('#LetterMessage').css("display", "none");
    $(".js-file:last")[0].files = e.originalEvent.dataTransfer.files;
    $(".js-file:last").change();
  });
});